...
fig_nums(name = "Fig_S3.5", caption = "Abundance of Daphnia and Copepods (Number / 10L) over all tanks and weeks.", display = FALSE)
fig_nums(name = "Fig_S3.6", caption = "Daphnia abundance with modeled temperature dependence; estimates and confidence intervals given in Table S3.3", display = FALSE)
fig_nums(name = "Fig_S3.7", caption = "Copepod abundance with modeled temperature dependence", display = FALSE)
```
...
m1D <- lme(log(abundance.Daphnia+1) ~ 1 + I(invTT - mean(invTT))*trophic.level, random = ~ 1|Tank, data = data.N)
qqnorm(resid(m1D))
# proceed with alternate fixed effects models
m1Da <- lme(log(abundance.Daphnia+1) ~ 1 + trophic.level, random = ~ 1|Tank, data = data.N)
m1Db <- lme(log(abundance.Daphnia+1) ~ 1 + I(invTT - mean(invTT)), random = ~ 1|Tank, data = data.N)
m1Dc <- lme(log(abundance.Daphnia+1) ~ 1, random = ~ 1|Tank, data = data.N)
m1Dd <- lme(log(abundance.Daphnia+1) ~ 1 + I(invTT - mean(invTT)) + trophic.level, random = ~ 1|Tank, data = data.N)

resD <- model.sel(m1D, m1Da, m1Db, m1Dc, m1Dd)
```
